...
class SummarizationModel(object):
    ...
    def _add_placeholders(self):
        ...
        # encoder part
        self._enc_batch = tf.placeholder(tf.int32, [sett.batch_size, None], name='enc_batch')
        self._enc_lens = tf.placeholder(tf.int32, [sett.batch_size], name='enc_lens')
        self._enc_padding_mask = tf.placeholder(tf.float32, [sett.batch_size, None], name='enc_padding_mask')
        if sett.using_pointer_gen:
            self._enc_batch_extend_vocab = tf.placeholder(tf.int32, [sett.batch_size, None], name='enc_batch_extend_vocab')
            self._max_art_oovs = tf.placeholder(tf.int32, [], name='max_art_oovs')
            
        # decoder part
        self._dec_batch = tf.placeholder(tf.int32, [sett.batch_size, sett.max_dec_steps], name='dec_batch')
        self._target_batch = tf.placeholder(tf.int32, [sett.batch_size, sett.max_dec_steps], name='target_batch')
        self._dec_padding_mask = tf.placeholder(tf.float32, [sett.batch_size, sett.max_dec_steps], name='dec_padding_mask')
        
        if sett.mode == "decode" and sett.using_coverage:
            self.prev_coverage = tf.placeholder(tf.float32, [sett.batch_size, None], name='prev_coverage')
    ...
    def _add_encoder(self, encoder_inputs, seq_len):
        ...
        with tf.variable_scope('encoder'):
            cell_fw = tf.contrib.rnn.LSTMCell(sett.hidden_dim, initializer=self.rand_unif_init, state_is_tuple=True)
            cell_bw = tf.contrib.rnn.LSTMCell(sett.hidden_dim, initializer=self.rand_unif_init, state_is_tuple=True)
    ...
    def _add_decoder(self, inputs):
        ...
        cell = tf.contrib.rnn.LSTMCell(sett.hidden_dim, state_is_tuple=True, initializer=self.rand_unif_init)        
        ...
        prev_coverage = self.prev_coverage if sett.mode == "decode" and sett.using_coverage else None
        
        #
        outputs, out_state, attn_dists, p_gens, coverage = attention_decoder(
                inputs, self._dec_in_state, self._enc_states, self._enc_padding_mask,
    ...
    def _calc_final_dist(self, vocab_dists, attn_dists):
        ...
        with tf.variable_scope('final_distribution'):
            ...
            extended_vsize = self.vocab.size() + self._max_art_oovs # the maximum (over the batch) size of the extended vocabulary
            extra_zeros = tf.zeros((self.settings.batch_size, self._max_art_oovs))
            ...
            batch_nums = tf.range(0, limit=self.settings.batch_size) # shape (batch_size)
            ...
            shape = [self.settings.batch_size, extended_vsize]
    ...
    # add the whole seq2seq model
    def _add_seq2seq(self):
        """Add the whole sequence-to-sequence model to the graph."""
        sett = self.settings
        vsize = self.vocab.size() # size of the vocabulary
        
        with tf.variable_scope('seq2seq'):
            # Some initializers
            self.rand_unif_init = tf.random_uniform_initializer(-sett.rand_unif_init_mag, sett.rand_unif_init_mag, seed=123)
            self.trunc_norm_init = tf.truncated_normal_initializer(stddev=sett.trunc_norm_init_std)
            
            # Add embedding matrix (shared by the encoder and decoder inputs)
            with tf.variable_scope('embedding'):
                embedding = tf.get_variable('embedding', [vsize, sett.emb_dim], dtype=tf.float32, initializer=self.trunc_norm_init)
            ...
            with tf.variable_scope('output_projection'):
                w = tf.get_variable('w', [sett.hidden_dim, vsize], dtype=tf.float32, initializer=self.trunc_norm_init)
                # w_t = tf.transpose(w)
            ...
            if sett.mode in ['train', 'eval']:
                ...
                with tf.variable_scope('loss'):
                    if sett.using_pointer_gen:
                        ...
                        self._loss = _mask_and_average(loss_per_step, self._dec_padding_mask)
                    else: # baseline model
                    ...
                    if sett.using_coverage:
                        with tf.variable_scope('coverage_loss'):
                            self._coverage_loss = _calculate_coverage_loss(self.attn_dists, self._dec_padding_mask)
                            tf.summary.scalar('coverage_loss', self._coverage_loss)
                        #
                        self._total_loss = self._loss + sett.cov_loss_wt * self._coverage_loss
            ...
            if sett.mode == "decode":
                ...
                topk_probs, self._topk_ids = tf.nn.top_k(final_dists, sett.batch_size*2) # take the k largest probs. note batch_size=beam_size in decode mode
...
#
def _mask_and_average(values, padding_mask):
    """
    """
...
def _calculate_coverage_loss(attn_dists, padding_mask):
    """
    """
    ...
    #
    coverage_loss = _mask_and_average(covlosses, padding_mask)
