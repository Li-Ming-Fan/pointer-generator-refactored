...
from vocab import UNKNOWN_TOKEN, START_DECODING, STOP_DECODING
...
#
def run_beam_search(model, batch, vocab, settings):
    ...
    hyps = [Hypothesis(tokens=[vocab.word2id(START_DECODING)],
                       ...
                       attn_dists=[],
                       p_gens=[],
                       coverage=np.zeros([batch["src_seq"].shape[1]]) # zero vector of length attention_length
                       ) for _ in range(settings.beam_size) ]
    #
    results = [] # this will contain finished hypotheses (those that have emitted the [STOP] token)
    steps = 0
    while steps < settings.max_dec_steps and len(results) < settings.beam_size:
        ...
        for i in range(num_orig_hyps):
            ...
            for j in range(settings.beam_size * 2):  # for each of the top 2*beam_size hyps:
        ...
        for h in sort_hyps(all_hyps): # in order of most likely h
            if h.latest_token == vocab.word2id(STOP_DECODING): # if stop token is reached...
                ...
                if steps >= settings.min_dec_steps:
                    results.append(h)
            else: # hasn't reached stop token, so continue to extend this hypothesis
                hyps.append(h)
            if len(hyps) == settings.beam_size or len(results) == settings.beam_size:
...
def sort_hyps(hyps):
    ...
    """
    return sorted(hyps, key=lambda h: h.average_log_prob, reverse=True)
