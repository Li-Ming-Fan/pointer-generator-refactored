        ...
        with self._graph.as_default():
            ...
            if self.num_gpu == 1:
                ...
                if self.settings.use_metric:
                    self._metric_tensor = loss_tensors["metric"]
                #
                self.loss_train_tensor = loss_tensors["loss_train"]
                #
                # regularization
                ...
                with tf.variable_scope(vs_str):
                    for gid in range(self.num_gpu):
                        with tf.device("/gpu:%d" % gid), tf.name_scope("bundle_%d" % gid):
                            ...
                            self.loss_train_tensor += loss_tensors["loss_train"]
                            #
                            grads = self._opt.compute_gradients(loss_tensors["loss_train"])
                            grads_bundles.append(grads)
                            #
                            self.outputs_list.append(output_tensors)
                            self.lossputs_list.append(loss_tensors)
                            #
                #
                if self.settings.reg_lambda > 0.0:
                    loss_reg = tf.add_n( [tf.nn.l2_loss(v) for v in self.trainable_vars
                ...
                #           
                # grad sum
                grad_and_vars = ModelBaseboard.sum_up_gradients(grads_bundles)
                #
